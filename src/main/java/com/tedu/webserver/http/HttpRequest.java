...
public class HttpRequest {
	...
	/**
	 * 实例化HttpRequest使用的构造方法，需要将对应客户端的Socket传入，以便读取客户端发送过来的请求内容
	 * @param socket
	...
	private void parseRequestLine() throws EmptyRequestException{
		 ...
		 * 
		 * 解析请求行时，在获取拆分后的数组元素时可能会引发数组下标越界，这是由于Http协议
		 * 允许客户端发送一个空请求过来导致的。我们后面解决。
		 */
	...
	private void parseURL(){
		 ...
		 * 最终将每个参数的名字作为key,值作为value存入到parameter中。
	...
	private void parseHeaders(){
		 ...
		 * 3.每当读取一个消息头信息时应当按照": "拆分为两项，第一项为消息头名字，第二项为消息头对应的值，将名字作为key，
		 * 将值作为value存入到属性headers这个Map中
		 */
		//		String str;
	...
	private void parseContent(){
		...
		/*
		 * 当一个form表单以post形式提交时，那么表单中的内容会被包含在消息正文中。
		 * 通过消息头中是否含有Content-Type与Content-Length就可以得知是否含有
		...
		if(headers.containsKey("Content-Length")){
			...
			System.out.println("消息正文类型为："+type);
			/*
			 * 根据类型判断正文内容
			 */
